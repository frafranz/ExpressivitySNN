...
from src.utils_spiketime_linear import get_spiketime, get_spiketime_derivative # TODO: harmonize imports
...
class TestSpiketimeLinear(unittest.TestCase):
    def test_spiketime(self):
        ...
        output_spikes_solution = torch.tensor([
            [[(1+3*0)/3., (1+1*0)/1., 1e10], [(1+3*0+1*0.1)/4., (1+1*0+3*0.1)/4., (1+1*0.1)/1.]],
            [[(1+3*0)/3., (1+1*0)/1., 1e10], [(1+3*0+1*0.5)/4., (1+1*0+3*0.5)/4., (1+0*0+1*0.5)/1.]]
        ]) # n_batch x n_in x n_out 
        ...
        output_spikes = get_spiketime(input_spikes, input_weights, input_delays, neuron_params, device)
    ...
    def test_spiketime_with_delay(self):
        ...
        device = "cpu"
        output_spikes_solution = torch.tensor([
            [[(1+3*(0+0.1))/3., (1+1*(0+0.05))/1., 1e10], [(1+3*(0+0.1)+1*(0.1+0.01))/4., (1+1*(0+0.05)+3*(0.1+0.2))/4., (1+0*(0+0.3)+1*(0.1+0.5))/1.]],
            [[(1+3*(0+0.1))/3., (1+1*(0+0.05))/1., 1e10], [(1+3*(0+0.1)+1*(0.5+0.01))/4., (1+1*(0+0.05)+3*(0.5+0.2))/4., (1+0*(0+0.3)+1*(0.5+0.5))/1.]]
        ]) # n_batch x n_in x n_out 
    ...
    def test_spiketime_derivative(self):
        ...
        The same small SNN is used as in the previous tests.
        """
        ...
        dw, dt, dd = get_spiketime_derivative(input_spikes, input_weights, input_delays, neuron_params, device, output_spikes)
