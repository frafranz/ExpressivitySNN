...
class TestEqualtimeLayer(unittest.TestCase):
    def test_init(self):
        """
        Test the initialization of one layer of the SNN, with randomized weights and threshold.
        ...
        layer = EqualtimeLayer(input_features, output_features, sim_params, weights_init, threshold_init, device, bias)
    ...
    def test_forward_with_thresholds(self):
        """
        Test the forward propagation through one layer of the SNN, including thresholds.
        No bias input, i.e. an additional fixed-time input spike, is used here.
        """
        input_features = 2
        output_features = 3
        ...
        output_spikes_solution = torch.tensor([
            [(.5+3*(0)+1*(0.1))/4., (1+1*(0)+3*(0.1))/4., (2+0*(0)+1*(0.1))/1.],
            [(.5+3*(0))/3., (1+1*(0)+3*(0.5))/4., (2+0*(0)+1*(0.5))/1.]
        ])
        assert_close(output_spikes,output_spikes_solution)
